<script lang="ts">
    /**
     * 开关按钮组件
     * 支持三种样式：apple（苹果 pill 滑块）、hmos（鸿蒙 pill 滑块）、classic（经典 N/Y 按钮）
     * 样式根据 toggleStyle 设置自动切换，默认为 auto（自动检测设备）
     */
    import { KPASettings } from "#/settings.svelte";
    import { isAppleDevice, isHMOSDevice } from "#/userData";
    ...
    const resolvedStyle = $derived(
        KPASettings.toggleStyle === "auto"
            ? (isAppleDevice() ? "apple" : isHMOSDevice() ? "hmos" : "classic")
...
{:else if resolvedStyle === "hmos"}
    <!-- 鸿蒙风格：pill 形轨道 + 圆形滑块，鸿蒙蓝配色 -->
    <!-- svelte-ignore a11y_click_events_have_key_events -->
    <div
        class="toggle-wrapper hmos {className}"
        class:wide
        class:disabled
        role="switch"
        aria-checked={checked}
    ...
    >
        {#if bgText}
            <span class="toggle-label">{bgText}</span>
        {/if}
        <div class="toggle-track-hmos" class:on={checked}>
            <div class="toggle-knob-hmos"></div>
        </div>
        {#if onText && offText}
            <span class="toggle-state-text">{checked ? onText : offText}</span>
        {/if}
...
<style>
    ...
    .toggle-wrapper.hmos.wide {
        display: flex;
        width: 100%;
        justify-content: space-between;
    }
    .toggle-wrapper.hmos.wide .toggle-label {
        flex: 1;
        min-width: 0;
        white-space: normal;
        word-break: break-all;
    ...
    .toggle-track-hmos.on .toggle-knob-hmos {
        transform: translateX(18px);
    }

    .toggle-wrapper.hmos:focus-visible .toggle-track-hmos {
        outline: 2px solid var(--color-primary);
        outline-offset: 2px;
    }
